...
class EmailAnalyzer:
    ...
    def load_data(self):
        ...
        for col in ['SenderEmail', 'Subject', 'Label']: 
    ...
    def analyze_emails_and_usernames_domains(self):
        if 'SenderEmail' not in self.df.columns or self.df["SenderEmail"].empty:
            print("Skipping email/username/domain analysis: 'SenderEmail' column missing or empty.")
            return
    ...
    def perform_email_clustering_and_pca(self):
        ...
        for i in range(n_clusters_to_use):
            ...
            if "Cluster" in self.df.columns and pd.api.types.is_numeric_dtype(self.df.get('Cluster', pd.Series(dtype=float))) and "SenderEmail" in self.df.columns: # Check df['Cluster'] type robustly
                cluster_df_rows = self.df[(self.df["Cluster"] == i) & (self.df["SenderEmail"].astype(str).str.strip() != "")]
                if not cluster_df_rows.empty:
                    cluster_email_domains_list = []
                    for email_str_in_cluster in cluster_df_rows["SenderEmail"]:
    ...
    def perform_virustotal_analysis(self):
        ...
        to_scan = list(Counter(valid_domains_for_vt).keys())[:10]
    ...
    def identify_high_risk_emails_from_label(self):
        ...
        self.high_risk_emails_identified = []
        if self.df.empty or 'Label' not in self.df.columns or 'SenderEmail' not in self.df.columns:
        ...
        try:
            ...
            high_risk_df = self.df[(labels_numeric == 1) & (self.df['SenderEmail'].astype(str).str.strip() != "")]
            for _, row in high_risk_df.iterrows():
                email = str(row['SenderEmail']).strip(); original_label = row['Label']
    ...
    def generate_report(self, username):
        ...
        report_filename = f"analysis_report_{safe_username}_{timestamp}.txt"
        report_path = os.path.join(REPORT_FOLDER, report_filename)
        report_lines = [f"EMAIL SECURITY ANALYSIS REPORT\nGenerated by: {username} at {timestamp}\n",
                        f"Analyzed file: {os.path.basename(self.file_path)}\n",
                        f"Total rows in CSV: {len(self.df) if not self.df.empty else 0}\n"]
        
        report_lines.append("\n=== DOMAIN ANALYSIS ===")
        report_lines.append(f"Found {self.num_unique_domains} unique domains (valid format from parsed emails)")
        report_lines.append("\nTop 10 sender domains:")
        if self.top_10_domains:
            for i, (d,c) in enumerate(self.top_10_domains,1): report_lines.append(f"  {i}. {d}: {c} emails")
        else: report_lines.append("  No valid domain data.")
        report_lines.append(f"\nDomain distribution entropy: {self.domain_entropy:.2f}")
        if self.num_unique_domains > 0: report_lines.append(f"  (Max possible: {self.max_possible_domain_entropy:.2f})")
        ...
        perc_act = (self.emails_with_actions_count / self.total_subjects_analyzed_for_actions * 100) if self.total_subjects_analyzed_for_actions > 0 else 0
        report_lines.append(f"Found {self.emails_with_actions_count} emails ({perc_act:.1f}%) with action phrases in {self.total_subjects_analyzed_for_actions} subjects.")
        report_lines.append("\nTop action phrases:")
        if self.action_phrases_data:
            for i,(p,c,pct) in enumerate(self.action_phrases_data,1): report_lines.append(f"  {i}. '{p}': {c} ({pct:.1f}%)")
        else: report_lines.append("  No action phrases found.")
        ...
        report_lines.append("\n\n=== VIRUSTOTAL DOMAIN ANALYSIS (SAMPLE - Top 10) ===")
        if self.vt_scores:
            for d,s in self.vt_scores.items(): report_lines.append(f"  {d}: {s if isinstance(s,dict) else 'Error/Not Found'}")
        else: report_lines.append("  No VirusTotal results.")
...
def download_report():
    if "username" not in session or "report_path" not in session: return redirect(url_for("login"))
